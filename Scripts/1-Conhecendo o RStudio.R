...
#Agora, salve esse script na pasta Scripts com o nome "Meu_primeiro_script_26-04-24".
#Para abrir um script já existente, basta ir em File > Open File
#Arquivos de scripts do R costumam terminar em ".R".

#Para executar uma linha (ou função) do script, você pode clicar em Run (aqui em cima do lado direito) ou segurar Ctrl e apertar Enter
#Não se preocupe com a posição do cursor, ele pode estar em qualquer posição da linha
#Tente fazer isso aqui, usando as duas opções e observe o Console aqui embaixo
1 + 1
...
#Veja o que acontece se você não colocar # para fazer um comentário
1 + 1 Operação simples
...
#Acesse as opções: Tools > Global Options > Appearance > Editor Theme
#Acesse as opções: Tools > Global Options > Code > Edditing e marque a opção soft-wrap R source files
...
#Files (ver arquivos da pasta raiz), plots (grafucos), Packages (pacotes instalados e carregados), Help (ajuda com as funções: ???
#Explore as opções do Help
...
# Ctrl+Shift+C: transforma codigo em comentário e vice-versa
# CTRL+SHIFT+M: (%>%) operador pipe. Guarde esse atalho, você o usará bastante.
# Posicione o cursos sobre a função e tecle F1: pedir ajuda
# Posicione o cursos sobre a função e tecle F2: abrir código base da função
# Agora, abra o Script 2